...
namespace Test
{
    ...
    public class TestDB
    {
        ...
        {
            new DBUtil().Execute("delete from plus_file where id = @id", n);
        }
...
select *
from t_department";
            ArrayList data = new DBUtil().Select(sql);
            return data;
        }
        ...
        public ArrayList GetPositionsByDepartmenId(string departmentId)
        {
            ...
            ArrayList dataAll = new DBUtil().Select(sql);
...
where a.name like '%" + key + "%'";           
            ...
            ArrayList ages = new DBUtil().Select("select min(age) as minAge, max(age) as maxAge, avg(age) as avgAge from t_employee");
...
where a.name like '%" + key + "%'";
        ...
        public void DeleteEmployee(string userId)
        {
            ...
            user["id"] = userId;
            new DBUtil().Execute("delete from t_employee where id = @id", user);
        }
...
where id = @id";
            ...
            new DBUtil().Execute(sql, db_d);
        }

        public Hashtable SearchEmployeesByMultiSort(string key, int index, int size, ArrayList sortFields)
        {
            //System.Threading.Thread.Sleep(300);
...
on a.educational = d.id";
            ...
            if (sortFields.Count > 0)
            {
                ...
                for (int i = 0, l = sortFields.Count; i < l; i++)
                {
